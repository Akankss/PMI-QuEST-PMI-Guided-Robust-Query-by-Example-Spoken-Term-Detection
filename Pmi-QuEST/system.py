...
Usage
-----
    from pmiquest.system import TFIDFBaseline, HQuEST, PMIQuest
    from pmiquest.system import run_comparison
    ...
    def fit(self, matrix: np.ndarray) -> "HNSWIndex":
        ...
        if _HNSWLIB_AVAILABLE:
            ...
            self._index = _NearestNeighbors(
                n_neighbors=self.n_neighbors, metric="euclidean", algorithm="ball_tree"
            ).fit(self._matrix)
        return self
...
class PMIFilteredTFIDF:
    ...
    Parameters
    ----------
    tau_pmi       : float   PMI threshold τ (paper: 0.5)
    bigram_weight : float   α in combined vector (paper: 1.0)
...
class TFIDFBaseline:
    """TF-IDF QbE-STD baseline. Unigram TF-IDF + brute-force cosine."""
    def __init__(self):
    ...
    def run(self, query_seqs, ground_truth) -> Dict[str, float]:
        return evaluate([self.rank(q) for q in query_seqs], ground_truth)
...
class HQuEST:
    """
    H-QuEST: Unigram TF-IDF → HNSW → Smith-Waterman reranking.
    ...
    def __init__(
        ...
        self.sw_gap      = sw_gap
        self.tfidf        = UnigramTFIDF()
        self.hnsw         = HNSWIndex(n_neighbors=C)
...
class PMIQuest:
    ...
    ----------
    tau_pmi : float   PMI threshold τ (paper: 0.5)
    alpha   : float   Bigram weight α (paper: 1.0)
    ...
    def __init__(
        ...
        self.sw_gap      = sw_gap
        self.min_count   = min_count
        self.pmi_tfidf    = PMIFilteredTFIDF(tau_pmi, alpha, min_count)
        self.hnsw         = HNSWIndex(n_neighbors=C)
        self._corpus_seqs: List[List[int]]      = []
        self._matrix:      Optional[np.ndarray] = None
...
def run_comparison(
    ...
    if verbose:
        ...
        print("=" * 70)
        print(f"  Corpus: {len(corpus_seqs)} docs  |  Queries: {len(query_seqs)}")
        if not _HNSWLIB_AVAILABLE:
            print("  WARNING: hnswlib not installed; using sklearn fallback.")
        print()

    for tag, label, system in [
        ("tfidf_baseline", "[1/3] TF-IDF Baseline",       TFIDFBaseline()),
        ("hquest",         "[2/3] H-QuEST",                HQuEST(C=200)),
        ("pmiquest",       "[3/3] PMI-QuEST (proposed)",   PMIQuest(tau_pmi=0.5, alpha=1.0, C=200)),
    ]:
        if verbose:
            print("-" * 70)
            print(label)
        ...
        m = system.run(query_seqs, ground_truth)
        m["time"] = time.time() - t0
        results[tag] = m
        if verbose:
            print(f"  {_fmt(m)}  [{m['time']:.1f}s]")
